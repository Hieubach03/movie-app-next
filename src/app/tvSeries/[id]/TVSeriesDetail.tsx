...
function TVSeriesDetail({ movie, cast, videos }: { movie: Movie, cast: Cast[], videos: any[] }) {
    return (
        <div>
            <div className='relative flex justify-center items-center w-full min-h-[100vh] bg-center bg-cover pb-40 lg:pb-0' style={{ backgroundImage: `url("https://image.tmdb.org/t/p/original${movie.backdrop_path}")` }}>
                ...
                <div className='absolute z-2 flex items-center px-6 lg:px-15 gap-x-10 w-full'>
                    ...
                    <div className='text-white flex flex-col gap-y-10 w-full mt-40'>
                        <h2 className='text-5xl md:text-7xl font-semibold'>{movie.name}</h2>
                        <div className='flex flex-wrap gap-y-3 gap-x-3'>
                            {movie.genres.map((genre) => (
                                <div key={genre.id} className='border-3 rounded-full px-5 py-1'>
                                    <p>{genre.name}</p>
                                </div>
                            ))}
                        </div>
                        <p className='text-justify text-[15px] lg:text-[20px] line-clamp-4'>{movie.overview}</p>
                        <div className='overflow-x-scroll scrollbar-hide'>
                            <h4 className='mb-3 text-2xl'>Casts</h4>
                            <div className='flex flex-wrap flex-row gap-x-5 gap-y-5'>
                                {cast.map((actor) => (
            ...
            {videos.length > 0 && (
                <div className='text-white px-6 lg:px-15 mt-20'>
                    <h3 className='text-2xl font-semibold mb-4'>Videos</h3>
                    <div>
                        {videos.map((video) => (
                            <div key={video.id} className='space-y-3 mb-10'>
                                <h4 className='md:text-xl font-medium'>{video.name}</h4>
                                <div className=''>
                                    <iframe
